...
export const UserMedia: React.FC<UserMediaProps> = ({ user, stream, isTiny, onClick, className = "" }) => {
  ...
  // Dynamic Scale Logic
  // We cap the scaling to prevent it from covering too much (max 1.3x)
  const dynamicScale = isTiny ? 1 : 1 + Math.min(volume / 100, 0.3); 
  ...
  // Optimized sizes:
  // Tiny: 32px (w-8)
  // Normal Mobile: 40px (w-10) -> Small enough not to obscure
  // Normal Desktop: 64px (w-16) -> Large enough for detail
  ...
  return (
    <div 
      onClick={onClick}
      className={`relative rounded-full transition-all duration-200 ease-out bg-black group cursor-pointer aspect-square shrink-0 ${sizeClasses} ${className}`}
      style={{
    ...
    >
      ...
      <div className="relative w-full h-full rounded-full overflow-hidden border-2 border-stadium-700 bg-stadium-800 shadow-md">
        {(!user.isVideoOff && stream) ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-stadium-700 text-white">
                <div className={`${isTiny ? 'text-[10px]' : 'text-sm lg:text-lg'} font-bold`}>
      ...
      {/* Hover Name Tag (Desktop Only) */}
      <div 
        className={`hidden lg:block absolute -bottom-8 left-1/2 -translate-x-1/2 bg-black/80 border border-white/10 px-2 py-1 rounded-md backdrop-blur-sm transition-opacity whitespace-nowrap z-50 pointer-events-none opacity-0 group-hover:opacity-100`}
      >
